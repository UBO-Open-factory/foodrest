  ...
  if (cfg.begin(fileName, CONFIG_LINE_LENGTH)) {
    ...
    while (cfg.readNextSetting()) {
      ...
      if (cfg.nameIs("InitialisationUsine")) {
        ...
        myConfig.calibrationFactor = cfg.getIntValue() / 100; // Get integer

        // Lecture de la valeur de tarage de la balance
      } else if (cfg.nameIs("valeurDeTarage")) {
        myConfig.valeurDeTarage = cfg.getIntValue() / 100 ; // Get integer

        // Lecture de la valeur initiale de tarage de la balance
      } else if (cfg.nameIs("laValeurDeTarageInitiale")) {
        myConfig.laValeurDeTarageInitiale = cfg.getIntValue() / 100; // Get integer
        ...
        myConfig.poidOld = cfg.getIntValue() / 100; // Get integer
